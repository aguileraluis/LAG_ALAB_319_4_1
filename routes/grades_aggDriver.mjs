...
router.get("/learner/:id/avg-class", async (req, res) => {
  ...
  await collection.createIndex({ learner_id: 1, avg : 1 }, (err, result) => {
    if (err) {
      console.log(err);
    } else {
      console.log(result);
  ...
  validator: {
    $jsonSchema: {
      properties: {
        learner_id: {
          minimum: 0; 
          description: "Your id must be between 0 and 300"
        }
      }
    }
  }
...
// Daniel Lin helped me get this function together.
router.get("/stats", async (req, res) => {
  ...
  if (!result) res.send("Not found").status(404);
    ...
    res.send({ totalLearners, learnersAbove70, percentageAbove70 }).status(200);
});
...
router.get("/stats/:id", async (req, res) => {
  ...
  const totalLearners = (await collection.distinct("class_id")).length;
  const learnersAbove70 = result.length;
  const percentageAbove70 = (learnersAbove70 / totalLearners) * 100;
  if (!result) res.send("Not found").status(404);
  else
